...
// Simple replacement for qs.stringify using native URLSearchParams
const stringify = (obj) => {
    if (!obj) return '';
    return new URLSearchParams(obj).toString();
};

export const Athlete = (athleteId) => ({
...
export default {
    ...
    athlete_splits: (athleteId, props) => `${_web}/athletes/${athleteId}/splits?${stringify(props)}`,

    leaders: (props) => `${_web}/leaders?${stringify(props)}`,
    ...
    teams: (args) => `${_site}/teams${args ? '?' + stringify(args) : ''}`,
